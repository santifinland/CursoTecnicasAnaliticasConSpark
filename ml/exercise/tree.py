...
def train(spark):
    ...
    label_indexer = StringIndexer(inputCol="income", outputCol="indexedLabel").fit(data)

    # Automatically identify categorical features, and index them.
    # We specify maxCategories so features with > 4 distinct values are treated as continuous.
    assembler = VectorAssembler(inputCols=["education-num"], outputCol="features").transform(data)
    feature_indexer = \
        VectorIndexer(inputCol="fnlwgt", outputCol="indexedFeatures").fit(data)
    ...
    pipeline = Pipeline(stages=[label_indexer, assembler, feature_indexer, dt])
